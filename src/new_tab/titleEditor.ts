...
type Methods = {
  ...
  focusLast(focus?: boolean): Promise<void>
}
type Computed = {
  widget: TitleWidget
}
...
> = {
  ...
  methods: {
    addMessage() {
      if (this.widget.type != "List") return
      this.widget.messages.push("A nice message")
      this.focusLast()
    },
    removeMessage(i: number) {
      let widget = this.widget
      if (widget.type != "List" || !widget.messages) return

      let messages = widget.messages
      if (messages[i].length != 0) return

      if (messages.length != 1) messages.splice(i, 1)
      this.focusLast(true)
    },
    async focusLast(focus = false) {
      await this.$nextTick()
      let inputs = this.$refs.messages as HTMLInputElement[]
